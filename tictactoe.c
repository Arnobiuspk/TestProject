...
/* 
A board is represented by an signed integer such that each position takes 2 bits: 
0 for blanks, 1 for player_X and 2 for player_O. 
The board is accessed by a cell index, which is in [0..8].
*/

int getCell( int cell, unsigned int board )
// return the value of the board at position cell.
{
...
{
  // put player at position cell in the board
  *board |= (player << (cell*2));
  return;
}
  

char convert( int cell, unsigned int board )
{
  int contents = getCell(cell, board);
  if      (contents == X_PLAYER) return 'X';
  else if (contents == O_PLAYER) return 'O';
  return (char)(((int)'0')+cell);
}
