...
#define main_beta         1000001
#define main_alpha		 -1000001
...
int evaluateMaxMove( unsigned int *board, int depth , int alpha , int beta);

int evaluateMinMove( unsigned int *board, int depth , int alpha , int beta )
{
  int value, x, y;
  ...
  */
  ...
  for (x = 0; x < MAX_BOARD_SIZE; x++)
	...
	if (scores2[x][y] == MAX_INFINITY) {
	  ...
	  min = evaluateMaxMove( board, depth+1,alpha ,beta); 
	  resetCell( x, y, board);
	  return min;
	}
      }
  ...
  for (x = 0; x < MAX_BOARD_SIZE; x++)
	...
	putCell( X_PLAYER, x, y, board );
	value = evaluateMaxMove( board, depth+1,alpha , beta );
	resetCell(x, y, board);
	...
	if (value < min) {
	  ...
	  if (min == MIN_INFINITY) return min;
		...
		pruning3(&value,&beta);
        int choice1 = pruning2(&alpha,&beta);
        if(choice1 == 1){
        	return beta;
		}
  ...
  */
  ...
  for (x = 0; x < MAX_BOARD_SIZE; x++)
	...
	if (scores2[x][y] == MAX_INFINITY) {
	  ...
	  max = evaluateMinMove( board, depth+1 , alpha , beta); 
	  resetCell( x, y, board);
	  return max;
	}
      }
  ...
  for (x = 0; x < MAX_BOARD_SIZE; x++)
	...
	putCell( O_PLAYER, x, y, board );
	value = evaluateMinMove( board, depth+1 , alpha , beta);
	resetCell( x, y, board);
	...
	if (value > max) {
	  ...
	  pruning1(&value ,&alpha);
	  int choice = pruning2(&alpha,&beta);
	  if(choice == 1){
	  	return alpha;
	  }
...
void getComputerMove( unsigned int *board )
{
  ...
  if (moves == 1) {
    ...
    value = evaluateMaxMove( board, 0 , main_alpha, main_beta);
